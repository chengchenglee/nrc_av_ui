...
class Box(object):
    ...
    def inside(self, point, axes=range(3), tolerance=None):
        ...
        Args:
          ...
          axes: A list of axis indices. For example, range(2) test for point inside top-down projection of the box
          tolerance: a list of tolerance for each axis, zero by default
        ...
        if tolerance is None:
            tolerance = [0, 0, 0]
        inv_trans = self.inv_transformation
        scale = self.scale
        point_w = np.matmul(inv_trans[:3, :3], point) + inv_trans[:3, 3]
        for i in axes:
            if abs(point_w[i]) > scale[i] / 2. + tolerance[i]:
                return False
        return True
    ...
    @property
    def inv_transformation(self):
        if self._inv_transformation is None:
            self._inv_transformation = np.linalg.inv(self.transformation)
        return self._inv_transformation
