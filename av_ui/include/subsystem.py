...
class Subsystem:
  ...
  def extract_launch_filename(self, command_str):
    """
    Extracts the full launch file path from a roslaunch command string. This function handles both the scenarios
    where full launch filepath is present in the command and also where the command references package name.

    Parameters:
    - command_str (str): A string that starts with 'roslaunch' and includes a launch file.

    Returns:
    - str or None: Full path to the launch file, or None if conditions aren't met.
    """
    if not command_str.startswith("roslaunch"):
      return None
    ...
    if parts[1].endswith(".launch"):
      return parts[1]

    if len(parts) >= 3 and parts[2].endswith(".launch"):
      package_name = parts[1]
      relative_path = parts[2]
      try:
        package_path = rospkg.RosPack().get_path(package_name)
        return os.path.join(package_path, 'launch/' + relative_path)
  ...
  def create_launch_file_with_machineDefs(self, input_filename, lines_to_insert, printDebug):
    """
    Inserts a list of strings after the <launch> line in the given file and saves the result to a new file.
    The new file name is generated by inserting '_avagent' before the .launch extension.
    If the output file already exists, the function does nothing.

    Parameters:
    - input_filename (str): Path to the input .launch file.
    - lines_to_insert (list of str): Lines to insert after the <launch> line.
    - printDebug whether to print debug info

    Returns:
    - str: Path to the output file, or None if the file already exists.
    """
    base, ext = os.path.splitext(input_filename)
    if ext != '.launch':
      raise ValueError("Input file must have a .launch extension")

    output_filename = f"{base}_avagent{ext}"

    if os.path.exists(output_filename):
      if printDebug: print(f"Output file '{output_filename}' already exists. Skipping modification.")
    ...
    for line in lines:
      ...
      if not inserted and '<launch>' in line:
  ...
  def updateLaunchFilesWithMachineDefs(self, machineDefs, printDebug):
    for cmd in self.commands:
      launch_file = self.extract_launch_filename(cmd.command)
      if launch_file:
        new_filename = self.create_launch_file_with_machineDefs(launch_file, machineDefs, printDebug)
        if new_filename:
          # new filename has _avagent suffix before extension
          cmd.command = cmd.command.replace('.launch', '_avagent.launch')
          if printDebug: print("launch command after modification: ", cmd.command)
          print("launch command after modification: ", cmd.command)
