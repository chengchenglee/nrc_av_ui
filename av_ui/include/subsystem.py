  ...
  def updateLaunchFilesWithVehicleTypeConfig(self, machineDefs, defaultArguments, printDebug):
    for cmd in self.commands:
      ...
      if orig_launch_file:
        ...
        # create a new launch file with _avagent appended to the filename
        base, ext = os.path.splitext(orig_launch_file)
        if ext != '.launch':
          raise ValueError("Input file must have a .launch extension")
        new_launch_file = f"{base}_avagent{ext}"
        
        # read original launch file content
        ...
        for line in lines:
          ...
          if not inserted and '<launch>' in line:
            ...
            for insert_line in machineDefs:
              new_lines.append('  ' + insert_line + '\n')       
            new_lines.append('\n') # Extra empty line
            # add deafult arguments
            for insert_line in defaultArguments:
              new_lines.append('  ' + insert_line + '\n')
            new_lines.append('\n') # Extra empty line
            inserted = True

        with open(new_launch_file, 'w') as file:
          file.writelines(new_lines)

        # update the command with the new launch filename
        if new_launch_file:
